...
func (a *VisionAdapter) Forward(ctx ml.Context, hiddenStates ml.Tensor, opts *VisionOptions) ml.Tensor {
	patches, tiles := hiddenStates.Dim(1), hiddenStates.Dim(2)
	patchSize := int(math.Sqrt(float64(patches)))

	hiddenStates = hiddenStates.Reshape(ctx, hiddenStates.Dim(0), patchSize, patchSize, tiles)

	hiddenStates = hiddenStates.Reshape(ctx, int(float32(hiddenStates.Dim(0))/opts.pixelShuffleRatio), int(float32(hiddenStates.Dim(1))*opts.pixelShuffleRatio), hiddenStates.Dim(2), tiles)
	hiddenStates = hiddenStates.Permute(ctx, 0, 2, 1, 3).Contiguous(ctx)

	hiddenStates = hiddenStates.Reshape(ctx, int(float32(hiddenStates.Dim(0))/opts.pixelShuffleRatio), hiddenStates.Dim(1), int(float32(hiddenStates.Dim(2))*opts.pixelShuffleRatio), tiles)
	hiddenStates = hiddenStates.Permute(ctx, 0, 2, 1, 3).Contiguous(ctx)

	hiddenStates = hiddenStates.Reshape(ctx, hiddenStates.Dim(0), hiddenStates.Dim(1)*hiddenStates.Dim(2), tiles)
...
func (m *VisionModel) rotaryEmbedding(ctx ml.Context) (ml.Tensor, ml.Tensor) {
	...
	freqsX, freqsY := make([]float32, numPatches*freqDim/2), make([]float32, numPatches*freqDim/2)
	for i := range numPatches - 1 {
		for j := 0; j < freqDim; j += 2 {
			ropeFreq := math.Pow(float64(m.ropeTheta), float64(j)*2/float64(headDim))
			freqsX[int(i)*freqDim/2+j/2] = float32(float64(1+i-floorDiv(i, patchesPerSide)*patchesPerSide) / ropeFreq)
			freqsY[int(i)*freqDim/2+j/2] = float32(float64(1+floorDiv(i, patchesPerSide)) / ropeFreq)
		}
	}

	ropeFreqs, err := ctx.Input().FromFloatSlice(append(freqsX, freqsY...), freqDim/2, numPatches, 2)
	if err != nil {
		panic(err)
	}
